...
from transformers.modeling_outputs import BaseModelOutput, BaseModelOutputWithPooling, SequenceClassifierOutput, MaskedLMOutput, ImageClassifierOutput
...
class ViTSelfAttention(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.query = ILSLinear(config.hidden_size, self.all_head_size, bias=config.qkv_bias)
        self.key = ILSLinear(config.hidden_size, self.all_head_size, bias=config.qkv_bias)
        self.value = ILSLinear(config.hidden_size, self.all_head_size, bias=config.qkv_bias)
...
class ViTLayer(nn.Module):
    ...
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.layernorm_before = ILSLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.layernorm_after = ILSLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class ViTForImageClassification(ViTPreTrainedModel):
    def __init__(self, config: ViTConfig) -> None:
        ...
        # Classifier head
        self.classifier = ILSLinear(config.hidden_size, config.num_labels) if config.num_labels > 0 else nn.Identity()
