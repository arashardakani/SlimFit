...
val_loader = DataLoader(eval_dataset, collate_fn=data_collator, batch_size=batch_size)
        ...
        if i % 2000 ==1999:
            with torch.no_grad():
                ...
                for j, batch1 in enumerate(tqdm(val_loader)):
                    ...
                    metric.add_batch(predictions=outputs.logits.argmax(dim = -1), references=batch1["labels"] )
